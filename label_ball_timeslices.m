...
%% 參數
DATA_FN = 'laser_dump_ball.dat';          % 來源 t, angle(rad), range(m)
SAVE_FN = 'labeled_points_ball_timesliced.csv';
DT = 1.0;                                % 每窗秒數
...
t = T(:,1); ang = T(:,2); r = T(:,3);
ok = isfinite(t)&isfinite(ang)&isfinite(r)&r>0; t=t(ok); ang=ang(ok); r=r(ok);
...
% 準備輸出檔（首行以 # 註解，Python 端 reader 可忽略）
fid = fopen(SAVE_FN,'w'); assert(fid>0);
fprintf(fid,'# columns: t,angle,range,label\n'); fclose(fid);

%% 輕量 GUI
fig = figure('Color','w','Name','左鍵逐點；Enter 完成；r=沿用；s/Esc=跳過；q=結束', ...
...
for k = 1:nWin
    ...
    title(ax,sprintf('Window %d/%d | t∈[%.3f, %.3f) | 點數=%d', ...
    ...
    elseif strcmp(key,'r') && ~isempty(Pprev) && size(Pprev,1)>=3
        % 沿用上一窗（先 bbox 過濾再 inpolygon，快很多）
